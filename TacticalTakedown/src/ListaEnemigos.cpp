...
//Fin listaEnemigos, inicio ListaTorres/////////////////////////////////////////////////////////////////
...
bool ListaTorres::agregar(Enemigo* e)
{
	if (numero < MAX_ENEMIGOS) {
		for (int i = 0; i < numero; i++)
			if (lista[i] == e)
...
void ListaTorres::destruirContenido()
{
	for (int i = 0; i < numero; i++) // destrucción de esferas contenidas
		delete lista[i];
	numero = 0; // inicializa lista
...
void ListaTorres::eliminar(Enemigo* e)
{
	for (int i = 0; i < numero; i++)
		if (lista[i] == e)
		{
...
Enemigo* ListaTorres::operator[](int i)
{
	if (i >= numero)//si me paso, devuelvo la ultima
		i = numero - 1;
	if (i < 0) //si el indice es negativo, devuelvo la primera
...
void ListaTorres::miraPunto(Vector2D Objetivo)
{
	for (int i = 0; i < numero; i++)
		lista[i]->miraPunto(Objetivo);
}
